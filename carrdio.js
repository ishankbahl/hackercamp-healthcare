...
let knn;
const csvFilePath = 'cardio.csv'; // Data
const names = ['crp', 'total_cholestrol', 'LDL', 'HDL', 'Target']; // For header
...
function predict() {
    ...
    prompt.get(['CRP', 'Total Cholestrol', 'LDL("bad" Cholestrol)', 'HDL("good" Cholestrol)'], function (err, result) {
        if (!err) {
            for (var key in result) {
                temp.push(parseFloat(result[key]));
            }
            console.log(`With ${temp} -- Taget =  ${knn.predict(temp)}`);
        }
    });
}
